...
日期：2026-09-08。v0.5 已实现可配置 CE 任务 API 客户端，包含能力查询、模型版本匹配、任务提交、状态查询和输出链接；端点及请求／响应契约见 [生成 API](generation-panel.md#ce-api-契约-v1)。CE 服务实现、执行器、权重安装与部署仍需另行接入。

本文于 2026-09-05 建立总体架构。除上述客户端外，下文服务划分、项目 API、调度、资产存储和恢复流程仍属需求级设计，不表示已经部署。

生成面板可填写 API 根地址，也可从仓库 [.env.example](../.env.example) 的 `VITE_CE_API_BASE_URL` 设置默认值。该值是浏览器公开配置，不保存服务端凭据。自定义模型 JSON 只定义前端字段和命令；CE 必须注册相同模型 ID／版本，并负责工作目录、Python 环境、文件检查和 GPU 资源。

**[确定]** 最终应用在浏览器中使用，前后端分离，后端部署于具有计算资源的 CE 集群。**[建议]** 下文的服务划分、API、技术候选和容错策略用于落实这一要求；具体框架、集群接入方式和性能预算在实施时确认。CE 的调度器、节点网络、共享存储和 GPU 配额目前未知，不假定使用 Slurm、Kubernetes 或任何指定平台。
...
前后端分离指代码、接口和运行职责分离；不要求前端页面与 API 使用不同公网域名。建议由同一 HTTPS 入口转发页面和 API，减少浏览器访问和认证配置。入口部署在哪个节点、通过什么集群代理访问，取决于 CE 的实际网络规则。
...
| 应用服务 | 项目创建与注册、上传、资产索引、版本校验、轨迹提交、作业状态与结果发布、预设管理 | 是修改项目引用的统一入口；不相信浏览器提交的 ready 标记 |
| CPU / GPU 工作进程 | SAM 推理、深度或点云重建、点与 mask 关联、预览数据转换、全精度条件渲染、首帧生成、SymphoMotion 导出与推理 | 读取固定输入版本，写入独立作业输出目录，由应用服务校验后发布 |
...
实施前只需确认实际运行所必需的以下信息，无需在需求阶段预设调度平台：

- 前端与 API 的可访问入口，以及浏览器到 CE 的连接方式。
- 应用服务的常驻位置，计算节点能否访问应用服务及共享存储。
- 计算任务提交、查询、取消的实际接口；已有调度器或平台的资源申请约定。
- GPU 类型、显存、配额、并发限制，以及模型环境与权重的可用位置。
- 项目和 assets 的持久化根目录；临时盘、共享盘或对象存储的实际能力。

**[建议]** 用执行器接口隔离这些差异：`submit / status / cancel / collect_outputs`。API 服务本身不直接绑定某一种调度器，也不要求浏览器掌握集群登录凭据。互动分割与长时间视频生成可使用不同队列或资源池，具体取决于 CE 支持的能力。

## 5. API 粗边界

以下是职责示例，不是已经固定的端点、请求结构或 OpenAPI 契约。前端仅依赖应用 API；上游模型的脚本路径和参数由适配器封装。
...
| 作业执行 | 应用作业记录 + CE 执行器适配层 | 接入 CE 实际提供的提交、查询、取消方式，不预设调度产品 |
...
以上均为候选，没有因此引入具体服务采购、额外账号或已授权部署动作。框架、依赖版本和渲染格式将在实现阶段依据 CE 环境与兼容性要求核实。
