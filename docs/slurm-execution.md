...
2026-09-09 工作流扩展：`depth`、`sam2`、`associate`、`export` 和原生成任务共用同一 Store、Slurm 提交／查询／取消／输出发布实现。全局面板集中编辑公共 Bash 脚本和资源参数，ENVNAME 分别放在任务面板；提交时冻结脚本正文，后续全局编辑不改动已排队任务。后端将环境名解析为配置中的准确 prefix。真实 GPU 作业与回归状态见 [验收记录](gpu-validation.md)。

2026-09-09 后端补充：已新增 [技术方案](backend-implementation.md) 和 [生成/Slurm 后端与部署说明](backend-deployment.md)，HTTP 服务已监听 0.0.0.0:8000，39 项后端测试通过。默认脚本按用户确认改为显式加载 Conda hook、失败即停，保留资源配置与 ENVNAME/argv 协议；已保存脚本不自动改写。现有推理环境为 `symphomotion`，不重新安装，真实 GPU 联调仍待权重齐备。以下关于“本轮仅前端”的段落记录此前 v0.6 客户端阶段，当前建设与验证范围以部署说明为准。
