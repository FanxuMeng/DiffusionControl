...
日期：2026-09-09。v0.6 已增加 ENVNAME、可折叠脚本编辑／导入／下载与只读的完整 sbatch 命令预览，数据按命名配置隔离；当前交互与验证见 [Slurm 规格](slurm-execution.md)。

模型参数与“推理命令”继续双向同步，Slurm 包装单独由字段生成。环境和脚本不会混入模型 input box；导入 Bash 不在浏览器执行。提交前要求 API v2、匹配模型版本及 `slurm_sbatch_v1` 能力，旧 API v1 记录仅查询／导出。登录节点负责提交，计算节点负责推理；实际 CE 服务尚未部署。v0.5 基础交互见 [生成面板规格](generation-panel.md)。

生成草稿和提交快照属于当前项目；切换模型或运行配置恢复各自内容，不改变场景时间轴。解析／校验失败时保留输入并禁止提交；倒计时、录制、暂停保存期间锁定生成编辑与提交，已接受的 CE 作业不因录制状态改变而取消。

连接区域默认显示同源 `/api` 和能力检查按钮，地址输入收进默认折叠的“高级连接设置”。已有自定义地址继续显示；编辑地址或“恢复同源 /api”会取消当前能力检查、使旧连接失效，重新检查后才可提交。恢复操作保存 `/api`，不修改历史任务的 endpoint，也不自动发起推理。录制期间锁定地址编辑与恢复操作。地址优先级与部署边界见 [同源连接](slurm-execution.md#同源连接与高级设置)。

轨迹操作沿用已确认的相机局部平移、松键停止、独立主动横滚与 ±89° 俯仰规则，v0.4 增加滚轮调速。历史选择见 [决策清单](decisions.md)。
