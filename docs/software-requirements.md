...
状态：2026-09-09，已按先文档、后代码实现 v0.6：ENVNAME、作业脚本编辑／导入／下载、完整 sbatch 预览与 API v2 执行门禁。后端部署目标已确定为 CE 登录节点，模型推理由 Slurm 分配到计算节点；本轮未部署真实服务或提交 GPU 任务。当前规格与验证见 [Slurm 执行](slurm-execution.md)。

v0.5 的模型／命名运行配置和命令双向编辑保持有效，D24–D26 已确认，见 [生成面板规格](generation-panel.md)。CE 服务、权重部署与场景条件导出仍需接入。

D17–D21 保持有效。v0.4 的 D22／D23 当前采用标注的推荐方案，尚未视为用户确认；历史验证范围见 [v0.4 记录](changes-v0.4.md)。

本文使用三个标记：**[确定]** 为用户明确要求；**[建议]** 为补齐操作闭环的设计；**[待定 Dxx]** 对应 [决策清单](decisions.md)。上游事实以 [兼容性调研](research-compatibility.md) 为准。

## 本轮 v0.6 修订要求

1. **[确定]** 后端 HTTP 服务部署在 CE 登录节点，通过 `sbatch job.gpu ENVNAME=base <推理 argv>` 提交模型任务；模型在计算节点运行。
2. **[确定]** 生成面板增加 ENVNAME 输入与可折叠的 Bash 脚本编辑／导入／下载；完整 sbatch 命令只读展示，并随环境、脚本文件名和模型参数同步更新。
3. **[确定]** 环境、脚本文件名和全文随命名运行配置保存、复制和切换，提交时冻结进请求快照；无服务时仍可编辑、校验与导出。
4. **[实现约束]** 新请求使用 API v2，并要求服务显式声明 `slurm_sbatch_v1` 能力；旧 API v1 请求只保留历史查询与导出，不自动转换或重试。
5. **[确定]** 页面与后端采用同源入口，浏览器默认使用 `/api`；地址覆盖放入默认折叠的高级连接设置，支持恢复同源地址。修改连接设置不改变历史任务保存的服务地址；实际入口与后端仍需部署。

执行契约与验收以 [Slurm 提交规格](slurm-execution.md) 为准。

## 历史 v0.5 修订要求

1. **[确定 D24]** 副面板增加可折叠的 Diffusion 生成子面板，首批包含 SymphoMotion 与可导入的自定义模型 profile；模型定义决定参数类型、默认值、校验规则和命令入口。
2. **[确定]** 参数输入框与可编辑的推理命令双向同步。保留未完成的输入草稿，解析错误时说明原因并阻止使用旧参数执行。
3. **[确定 D26]** 模型 profile 与项目运行 profile 分开；每个项目可为一个模型保存多份可命名、可复制的运行配置，切换后恢复对应字段和命令。
4. **[确定 D25]** 通过可配置 CE 任务 API 查询模型能力、提交请求和查看作业；未连接时可编辑、校验和导出请求。前端不模拟模型生成成功，模型安装与执行由 CE 管理。

数据格式、接口与验收以 [生成面板规格](generation-panel.md) 为准。
...
该版实现及验证范围见 [v0.3 修订记录](changes-v0.3.md)。
...
- **[确定]** 最终应用在浏览器中实现，前后端分离；服务端部署于 CE 登录节点，经 Slurm 提交模型到计算节点。浏览器即时处理键鼠与轨迹采样，详见 [部署架构](deployment-architecture.md)。
...
主面板仅保留 2D、3D 两个视图；“预览／绘制”及“自由观察／跟随相机”是独立状态。切换到 3D 不自动录制。4D 仍指带时间的场景数据和构建步骤，不再是单独窗口或 tab。副面板固定从上到下为 Projects、Object Control、Camera Control、Diffusion 生成。
...
- **[确定]** 选中项目后读取 JSON，联动刷新物体、相机和生成面板，主面板默认加载首帧；几何只在所需视图加载。
...
| AC02 | 导入单项目与母目录均形成可选列表；点击项目同步刷新两个视图状态以及物体、相机和生成面板 |
...
| AC20 | 模型与命名运行配置切换后参数和命令一致，草稿与请求快照按项目保存；无 CE 可校验／导出，连接后按注册模型版本提交和查询真实任务，详见生成面板验收 |
| AC16 | SymphoMotion 导出通过目标版本 loader 的实际读取与条件投影验证；该项当前仅为后续集成验收要求 |

## 7. 建议实现顺序

1. 项目读写、空态、首帧导入和项目／物体／相机面板骨架；v0.5 已补充第四个生成面板及配置保存。
...
6. 按已实现的 API v2 客户端契约接入 CE 登录节点服务与 Slurm 执行器，注册模型定义与运行环境，验证真实任务及输出。

该顺序是开发建议，不削减上述功能范围。浏览器、前后端分离、CE 登录节点与 Slurm 执行方式已确定；具体服务框架、网络入口、路径和性能预算仍需依据 CE 环境落实。
