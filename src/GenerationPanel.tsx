...
import {
  ...
  selectProjectProfile, updateCommand, updateField,
...
import {
  ...
  readApiEndpoint, saveApiEndpoint, submitGenerationJob,
} from './generation/api';
import type {
...
export default function GenerationPanel({ projectId, projectName, state, locked, onChange, onNotify }: GenerationPanelProps) {
  ...
  const compatible = !!model && hasProfile(connection, model.id, model.version);
  ...
  async function connect() {
    ...
    try {
      ...
      if (!base) throw new Error('请填写 CE 任务 API 根地址。');
      saveApiEndpoint(base);
      setEndpoint(base);
      setConnecting(true);
  ...
  function retry(submission: GenerationSubmission) {
    if (locked || submission.job || submission.rejection || !connection || connection.endpoint !== submission.endpoint || !hasProfile(connection, submission.request.profileId, submission.request.profileVersion)) return;
    void post(submission);
  }
  const disabledReason = locked ? '录制期间暂停编辑和提交。' : !inspection.canExecute ? '请修正参数或命令中的错误。' : !connection ? '连接 CE 后可执行；当前可导出请求。' : !compatible ? 'CE 未注册此模型 profile 的相同版本。' : unresolved ? '当前配置已有未结束或待确认的任务。' : '使用当前参数提交到 CE。';
  ...
  return <section className="dcp-panel generation-panel" aria-label="Diffusion 生成">
    ...
    <div id={`${uid}-body`} className="generation-body" hidden={collapsed}>
      ...
      {draft && <>
        <details className="generation-parameter-group" open>
          ...
          <div className="generation-fields">{fields.map((parameter) => {
            const fieldId = `${uid}-${parameter.key}`;
        ...
        <div className="generation-block generation-command-block">
          <label className="generation-label" htmlFor={`${uid}-command`}>推理命令 <span>可直接编辑</span></label>
          ...
          {inspection.issues.length > 0 ? <div className="generation-validation is-error" role="status"><strong>{inspection.commandParseable ? '参数校验未通过' : '命令无法解析'}</strong><ul>{inspection.issues.map((issue, index) => <li key={index}>{issue.message}</li>)}</ul></div> : <div className="generation-validation is-valid" role="status"><Check size={12} />参数与命令校验通过</div>}
        </div>
      </>}
      ...
      <div className="generation-block generation-api-block">
        <label className="generation-label" htmlFor={`${uid}-endpoint`}>CE 任务 API 根地址</label>
        <input id={`${uid}-endpoint`} value={endpoint} disabled={locked} type="text" inputMode="url" autoComplete="off" spellCheck={false} placeholder="https://ce.example.edu/api" onChange={(event) => editEndpoint(event.target.value)} />
        ...
        {connection && <p className={`generation-help ${compatible ? 'is-success' : 'is-warning'}`}>{compatible ? '服务已注册当前模型与版本。' : '已连接，但服务未注册当前模型与版本。'}</p>}
        <p className="generation-help">路径填写 CE 上可访问的位置。未连接时可校验和导出请求。</p>
        <div className="generation-actions">
      ...
      {state.submissions.length > 0 && <details className="generation-jobs" open>
        ...
        <div className="generation-job-list">{[...state.submissions].reverse().map((submission) => {
          ...
          const canRetry = !locked && !pending && !job && !rejection && connection?.endpoint === submission.endpoint && hasProfile(connection, request.profileId, request.profileVersion);
          return <article className="generation-job" key={request.requestId}>
            ...
            {!job && !pending && !rejection && <p className="generation-help">连接原服务后可重试；请求 ID 和内容保持不变。</p>}
            <div className="generation-job-actions"><button className="dcp-button" onClick={() => downloadJson(`${request.requestId}.request.json`, request)}><Download size={11} />请求快照</button>{!job && !rejection && <button className="dcp-button" disabled={!canRetry} title={`需连接 ${submission.endpoint} 并匹配原模型版本`} onClick={() => retry(submission)}><RefreshCw size={11} />{pending ? '提交中' : '重试提交'}</button>}</div>
